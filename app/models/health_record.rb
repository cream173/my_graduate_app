  ...
  def kampo_advice(manual_symptoms = nil)
    ...
    if feeling == 5 || stress_level >= 4
      if s_names.intersect?(["頭痛", "のぼせ"])
        return {
          type: "気逆（きぎゃく）",
          message: "ストレスで気が頭にのぼり、パンパンに張りがちです。今は難しいことは考えずに、ミントの香りを嗅いだり、足元を温めて高ぶりを下に降ろしてあげましょう。",
          foods: "トマト、セロリ、ピーマン、レタス、緑茶"}
      elsif s_names.intersect?(["胃のむかつき", "便秘"])
        return {
          type: "気滞（きたい）",
          message: "ストレスが胃腸の動きを邪魔しているのかもしれません。シトラス系の香りで巡りを整え、軽くお散歩してリフレッシュするとよいでしょう",
    ...
    if feeling == 3 || sleep_level >= 4
      if s_names.intersect?(["目の疲れ", "めまい"])
        return {
          type: "血虚（けっきょ）",
          message: "栄養が目や脳まで届かず、少しお疲れ気味のようです。夜は早めに照明を落とし、スマホをお休みして暗闇の中で心と目をじっくり休ませてあげましょう。",
    ...
    if s_names.intersect?(["肩こり", "冷え", "便秘", "頭痛"])
      return { 
        type: "瘀血（おけつ）", 
        message: "血の巡りが滞り、痛みやコリとして現れているかもしれません。締め付けの少ない楽な服に着替えて、ちょっとしたストレッチをして体をゆるめてみましょう。",
        foods: "納豆、ニラ、青魚、黒酢、にんにく"}
    end
